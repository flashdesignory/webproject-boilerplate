(function(){
	function Slider(id,node){
		...
		function thumbMove(event){
			if(event.touches)
			{
				_localX = event.touches[0].pageX - (_scrubberX + _node.offsetLeft + _node.parentElement.offsetLeft);
			...
			else
			{
				if(window.event != null){
					...
					_localX = event.clientX - (_scrubberX + _node.offsetLeft + _node.parentElement.offsetLeft) + xOffset;
					_localY = event.clientY - (_scrubberY + _node.offsetTop + _node.parentElement.offsetTop) + yOffset;
				} else {
